...
async def create_rank_card(member: discord.Member, xp: int, level: int, next_level_xp: int) -> BytesIO:
    ...
    avatar_bytes = await member.avatar.read() if member.avatar else await member.default_avatar.read()
    ...
    draw.text((110, 85), f'Level {level} - {xp}/{next_level_xp} XP', font=font_small, fill='white')
